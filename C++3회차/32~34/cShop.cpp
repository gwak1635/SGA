...
void cShop::SetStock()
{
	...
	item.nPrice = 25000;
	item.nAttribute = 50;
	item.strDesc = "유명한 락커가 입었던 가죽 잠바, 방어력 50상승";
	AddItem(item);
...
void cShop::MainPlay(LOCATION& m_eCurrLoc, cInventory& inven)
{
	...
	switch (nSelect)
	{
		...
		case 2:
		{
			...
			if (1 <= nSelect && nSelect <= this->GetItemList(eType).size()) {
				tagItem buyItem = this->PickItem(eType, nSelect - 1);
				inven.AddItem(buyItem);

				cout << endl << ">>>>>>>>>>>>>>>>>[" << buyItem.strName << "]을 구매하였습니다." << endl << endl;
				system("pause");
			}
		...
		case 3:
		{
			...
			if (1 <= nSelect && nSelect <= inven.GetItemList(eType).size()) {
				tagItem sellItem = inven.PickItem(eType, nSelect - 1);
				this->AddItem(sellItem);

				cout << endl << ">>>>>>>>>>>>>>>>>[" << sellItem.strName << "]을 판매하였습니다." << endl << endl;
				system("pause");
			}
		}
		break;
