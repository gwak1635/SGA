...
{
	if (loop == true)
		system->createStream(String::ToString(file).c_str(),
			FMOD_LOOP_NORMAL, NULL, &sound[sounds.size()]);
	else
		system->createStream(String::ToString(file).c_str(),
			FMOD_DEFAULT, NULL, &sound[sounds.size()]);
...
void Sounds::Play(string name, float volum)
{
	...
	for (iter; iter != sounds.end(); iter++, count++)
	{
		if (name == iter->first)
		{
			system->playSound(*iter->second, NULL, false,
				&channel[count]);
